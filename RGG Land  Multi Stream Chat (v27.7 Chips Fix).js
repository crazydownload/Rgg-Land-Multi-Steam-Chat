// ==UserScript==
// @name         RGG Land / Multi Stream Chat (v27.7 Chips Fix)
// @namespace    rgg.chat.sync
// @version      27.7
// @description  v27.7: Исправлен баг с появлением списка стримеров (chips) в обычном режиме. Теперь они создаются только в стене.
...
(function () {
  ...
  function gmPostJSON(url, headers, data, onOk, onFail) {
    ...
    if (typeof fetch === 'function') {
      fetch(url, { method: 'POST', headers: headers, body: data }).then(r => r.json()).then(onOk).catch(onFail);
    } else onFail();
  }
  ...
  function parseCheckersSeasons(html) {
    ...
    while ((m = re.exec(html))) {
      seasons.push({ name: 'RGG Шашки · Сезон ' + m[1], slug: 's' + m[1], range: parseRuRange(m[2]) });
    }
    return seasons;
  }

  function parsePlayersFromPayload(html) {
    const map = {};
    const loginRe = /\\"login\\":\\"([^\\"]+)\\"/g;
    let lm;
    while ((lm = loginRe.exec(html))) {
      const login = lm[1];
      const end = html.indexOf('},{\\"user\\":', lm.index);
      const win = html.slice(lm.index, end > 0 ? end : lm.index + 500);
      const g = win.match(/\\"currentGame\\":\\"((?:[^"\\\\]|\\\\.)*)\\"/);
      const o = win.match(/\\"isLive\\":(true|false)/);
      let game = g ? g[1] : null;
      if (game) {
        game = game.replace(/\\u([0-9a-fA-F]{4})/g, (_, h) => String.fromCharCode(parseInt(h, 16)));
        if (game === '') game = null;
      }
      map[login] = { game: game, online: o ? o[1] === 'true' : false };
    }
    return map;
  }
  ...
  function fetchTwitchGql(login, done) {
    ...
    gmPostJSON('https://gql.twitch.tv/gql',
      ...
      (j) => {
        ...
        if (!s) return done('OFFLINE');
        done((s.game && (s.game.displayName || s.game.name)) || null);
      },
      () => done(null)
    );
  }
  ...
  function applyGame(game, via) {
    ...
    else setGameText('—', 'Игра недоступна' + (via ? ' · ' + via : ''));
  }
  ...
  function updateGameBadge() {
    ...
    refreshEvent((map, label, ok) => {
      ...
      if (entry && entry.online && entry.game) { applyGame(entry.game, label); return; }
      fetchTwitchGame(myNick, (g) => { if (myNick === String(current).toLowerCase()) applyGame(g, 'twitch'); });
    });
  }
  ...
  function buildPanel() {
    const css = `
      ...
      #rggchat.rgg-wall .rgg-wall-open{display:none}
      /* В стене скрываем чипы (список стримеров), оставляем только игру */
      #rggchat.rgg-wall .rgg-chips{display:none}
      #rggchat.rgg-wall .rgg-top{cursor:default}
      #rggchat.rgg-wall .rgg-name{font-size:20px}

      /* Растягиваем бейдж игры на всю ширину в стене */
      #rggchat.rgg-wall .rgg-game {
        flex: 1;
      ...
      .rgg-body{flex:1;min-height:0;background:var(--bg);position:relative;overflow:hidden}
      .rgg-frame{width:100%;height:calc(100% + ${PLAQUE_SHIFT}px);border:0;display:block;background:var(--bg);color-scheme:dark;
        transform:translateY(-${PLAQUE_SHIFT}px);
    ...
    panel.innerHTML = `
      <div class="rgg-top">
        ...
        <span class="rgg-ver">v27.7${MODE === 'wall' ? '·wall' : ''}</span>
      ...
      <div class="rgg-row2">
        <span class="rgg-game" title="Текущая игра стримера">
    ...
    refs.body = panel.querySelector('.rgg-body');
  ...
  function refreshChips() {
    ...
    // --- ФИКС v27.7: Блокируем создание чипсов в режиме dock ---
    if (MODE !== 'wall') {
  ...
  function buildDom() {
    try {
      ...
      document.querySelectorAll('style[data-rgg]:not(#rggwallhide):not(#rgg-wall-hide)').forEach(n => n.remove());
      ensureWallHideStyle();
      refs = {};
      embeddedActive = false;
      buildPanel();
      ...
      console.log('[RGG-chat] v27.7 dom built · mode=' + MODE + ' · host=' + HOST);
    } catch (e) {
  ...
  console.log('[RGG-chat] v27.7 started · mode=' + MODE + ' · host=' + HOST);
})();