// ==UserScript==
// @name         RGG Land — чат + листочек + стена кнопкой (v21)
// @namespace    rgg.land.chat.sync
// @version      22.0
// @description  v22: фикс чёрного чата при старте (iframe не грузится в скрытой панели) + чат только встроенный/окно + матчи /live
...
(function () {
  ...
  const INSTANCE_KEY = '__rggChat_v21';
  ...
  /* ===== самоочистка предыдущего инстанса (и старого якоря от v20 при апгрейде) ===== */
  const timers = [];
  ...
  /* ===== единственный возврат-индикатор: тихий маркер виден, когда чат не встроен ===== */
  ...
  function enterEmbedded(width, nativeEl, ch) {
    ...
    void p.offsetHeight;            // ФИКС: форс-reflow — iframe получит реальный размер ДО загрузки Twitch-embed
    embeddedActive = true;
    applyReturnVisibility();        // маркер прячется, пока чат встроен
    setChat(ch);
  }

  /* ===== выход из встроенного: чат прячется (НЕ плавающий), маркер возвращается ===== */
  function exitEmbedded() {
    ...
    hidden = true;                                  // плавающего больше нет — только «спрятан»
    if (refs.panel) refs.panel.style.display = 'none';
    applyReturnVisibility();                        // тихий маркер в углу
    saveState();
  }
  ...
  /* ===== тихий маркер: индикатор «скрипт тут» + клик открывает чат окном-стеной ===== */
  ...
  function buildPanel() {
    const css = `
      ...
      #rggchat.rgg-wall .rgg-sz,#rggchat.rgg-wall .rgg-wall-open{display:none}
      ...
      /* чипы-вкладки — ТОЛЬКО в стене; на первом мониторе прячем, кнопки уезжают вправо */
    ...
    panel.innerHTML = `
      <div class="rgg-top">
        ...
        <span class="rgg-ver">v21${MODE === 'wall' ? '·wall' : ''}</span>
    ...
    if (MODE === 'dock') {
      // старт ВСЕГДА скрытым: не мешаем выбирать стримы; чат появится встроенным по листочку сайта
      hidden = true;
      if (refs.panel) refs.panel.style.display = 'none';
      applyReturnVisibility();   // тихий маркер в углу как индикатор «скрипт тут»
    }
    if (MODE === 'wall') {
  ...
  function tickBody() {
    ...
    // dock: чат только встроенный — появляется, когда сайт держит свою колонку открытой
    ...
    // ФИКС чёрного чата: в dock НЕ грузим iframe фоном, пока панель скрыта.
    // Раньше фоновый setChat ставил src в display:none контейнер → Twitch рендерил пусто
    // и кэшировал «чёрный» чат, а при встраивании early-return (current===ch) не давал
    // iframe перезагрузиться. Теперь iframe грузится ровно один раз — в enterEmbedded,
    // когда панель уже видима, и сразу правильного стримера. Желаемый канал берётся
    // через detectActive/lastBroadcast, фоновая загрузка для этого не нужна.
  }
  ...
  function buildDom() {
    try {
      ...
      const oldD = document.getElementById('rggdock'); if (oldD) oldD.remove();   // якорь от v20 при апгрейде
      ...
      console.log('[RGG-chat] v21 dom built · mode=' + MODE + ' · browser=' + detectBrowser());
    } catch (e) {
  ...
  function keyHandler(e) {
    if (MODE !== 'dock') return;
    // Alt+C = открыть чат отдельным окном-стеной (встроенный открывается листочком сайта)
  ...
  console.log('[RGG-chat] v21 started · mode=' + MODE + ' · browser=' + detectBrowser());
})();