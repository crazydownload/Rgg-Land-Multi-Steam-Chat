...
// @version      26.0
// @description  v26: игра из rgg.land (автоопределение эвента шашки/ЭГЛэнд по дате), длинный ник на 2 строки, WALL прячет 1-й экран, листочек = тумблер
...
(function () {
  ...
  /* ================= игра с rgg.land (вариант А: только официальный эвент) ================= */
  ...
  let eventCache = { ts: 0, map: {}, label: '—' };

  function gmGetText(url, onOk, onFail) {
  ...
  // "24 июля – 9 августа 2026" / "1 мая по24 мая 2026" -> [Date, Date] или null
  function parseRuRange(str) {
    if (!str) return null;
    const s = String(str).toLowerCase().replace(/\s+/g, ' ').trim();
    // ищем два месяца и числа вокруг них
    const months = Object.keys(RU_MONTHS).join('|');
    const re = new RegExp('(\\d{1,2})\\s+(' + months + ')\\s*(?:–|-|по)?\\s*(\\d{1,2})\\s+(' + months + ')\\s+(\\d{4})');
    const m = s.match(re);
    if (!m) return null;
    const d1 = new Date(+m[5], RU_MONTHS[m[2]] - 1, +m[1]);
    const d2 = new Date(+m[5], RU_MONTHS[m[4]] - 1, +m[3]);
  ...
  function todayInRange(range) {
    ...
    const a = new Date(range[0]); a.setHours(0, 0, 0, 0);
    const b = new Date(range[1]); b.setHours(23, 59, 59, 999);
    return t >= a && t <= b;
  }

  // список сезонов шашек: [{name, range, slug}] со страницы /checkers/
  function parseCheckersSeasons(html) {
    const seasons = [];
    // ищем блоки "RGG Шашки. Сезон N" + дата
    const re = new RegExp('RGG\\s*Шашки\\.\\s*Сезон\\s*(\\d+)[\\s\\S]{0,400}?(\\d{1,2}\\s+[а-яё]+\\s*[–\\-по]\\s*\\d{1,2}\\s+[а-яё]+\\s+\\d{4})', 'gi');
    let m;
    while ((m = re.exec(html))) {
      const slug = 's' + m[1];
      const range = parseRuRange(m[2]);
      seasons.push({ name: 'RGG Шашки · Сезон ' + m[1], slug: slug, range: range });
    }
    return seasons;
  }

   // таблица игроков шашек: ник -> игра (по карточкам; игра = блок text-text-secondary БЕЗ слова "очка")
  function parseCheckersPlayers(html) {
    const map = {};
    // каждая карточка начинается с аватарки: <img alt="НИК" ... class="MuiAvatar-img ...">
    const cardRe = /<img[^>]*alt="([^"]+)"[^>]*class="[^"]*MuiAvatar-img[^"]*"[^>]*>([\s\S]*?)(?=<img[^>]*class="[^"]*MuiAvatar-img|$)/gi;
    let cm;
    while ((cm = cardRe.exec(html))) {
      const nick = cm[1].trim();
      const body = cm[2];
      // все span внутри блоков text-text-secondary
      const segRe = /text-text-secondary[^>]*>[\s\S]*?<span>([^<]*)<\/span>/gi;
      let sm, game = null;
      while ((sm = segRe.exec(body))) {
        const txt = sm[1].trim();
        if (!txt) continue;
        if (/очк/i.test(txt)) continue;   // это очки — пропускаем
        game = txt;                          // это игра
      }
      if (nick && game) map[nick] = game;
    }
    return map;
  }

  // таблица игроков ЭГЛэнд: ник -> игра (со страницы /games, колонки "Участник" и "Текущая игра")
  function parseLandPlayers(html) {
    const map = {};
    // строки таблицы: ячейки <td>...</td>; берём 2-ю (участник) и 5-ю (текущая игра)
    ...
    while ((rm = rowRe.exec(html))) {
      ...
      while ((cm = cellRe.exec(rm[1]))) cells.push(cm[1].replace(/<[^>]+>/g, '').trim());
      // ищем строку, где есть участник и игра (пропускаем заголовок)
      if (cells.length >= 5 && cells[1] && cells[4] && /\w/.test(cells[1]) && !/^участник$/i.test(cells[1])) {
        map[cells[1]] = cells[4];
      }
    }
    return map;
  ...
  function applyGame(game, via) {
    if (game) setGameText(game, game + (via ? ' · ' + via : ''));
    else setGameText('—', 'Игра не указана в текущем эвенте' + (via ? ' · ' + via : ''));
  }

  function refreshEvent(done) {
    if (eventCache.ts && Date.now() - eventCache.ts < 5 * 60 * 1000) { done(eventCache.map, eventCache.label); return; }
    ...
    gmGetText(base + '/checkers/', (htmlC) => {
      ...
      if (active) {
        gmGetText(base + '/checkers/season/' + active.slug, (htmlP) => {
          const map = parseCheckersPlayers(htmlP);
          eventCache = { ts: Date.now(), map: map, label: active.name };
          done(map, active.name);
        }, () => { eventCache = { ts: Date.now(), map: {}, label: active.name }; done({}, active.name); });
        return;
      }
      // 2) ЭГЛэнд
      gmGetText(base + '/games', (htmlG) => {
        // дата эвента на странице /games
        const dateMatch = htmlG.match(/(\d{1,2}\s+[а-яё]+\s+(?:по\s*)?\d{1,2}\s+[а-яё]+\s+\d{4})/i);
        const range = parseRuRange(dateMatch ? dateMatch[1] : '');
        if (todayInRange(range)) {
          const map = parseLandPlayers(htmlG);
          eventCache = { ts: Date.now(), map: map, label: 'RGG Land' };
          done(map, 'RGG Land');
        } else {
          // межэвентье
          eventCache = { ts: Date.now(), map: {}, label: 'межэвентье' };
          done({}, 'межэвентье');
        }
      }, () => { eventCache = { ts: Date.now(), map: {}, label: '—' }; done({}, '—'); });
    }, () => { eventCache = { ts: Date.now(), map: {}, label: '—' }; done({}, '—'); });
  }

  function updateGameBadge() {
    refreshEvent((map, label) => {
      if (!current) { applyGame(null, label); return; }
      const game = map[current] || null;
      applyGame(game, label);
    });
  }
  ...
  function buildPanel() {
    const css = `
      ...
      #rggchat.rgg-wall .rgg-wall-open,#rggchat.rgg-wall .rgg-game{display:none}
      ...
      /* бейдж игры — 16px, перенос на несколько строк */
      ...
      /* шапка — ник переносится на 2 строки */
    ...
    panel.innerHTML = `
      ...
      <div class="rgg-row2">
        <span class="rgg-game" title="Текущая игра стримера (с rgg.land)">
    ...
    if (MODE === 'dock') {
      refs.top.addEventListener('pointerdown', e => {
        ...
        const up = () => { busy(false); saveState(); removeEventListener('pointermove', move); removeEventListener('pointerup', up); };
