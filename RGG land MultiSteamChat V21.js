...
// @version      21.0
// @description  v21: тихий маркер — единственный возврат (булавка удалена, ловушка невозможна) + чипы только в стене
...
(function () {
  ...
  let current = null, invertOn = true, hidden = false, siteMode = false;
  ...
  function saveState() {
    ...
    try { localStorage.setItem(SAVE_KEY, JSON.stringify({ ...lastGeom, inv: invertOn, hidden, site: siteMode })); } catch (e) {}
  }
  ...
  /* ===== единственный возврат: тихий маркер виден, когда чат скрыт и не встроен ===== */
  ...
  function setHidden(h) {
    hidden = h;
    if (embeddedActive) { applyReturnVisibility(); saveState(); return; }
  ...
  function updateXTitle() {
    if (refs.xBtn) refs.xBtn.title = 'Скрыть чат · вернуть тихим маркером в углу или Alt+C';
  }
  ...
  function exitEmbedded() {
    ...
    hidden = false;
    restoreGeom();
    if (refs.panel) refs.panel.style.display = 'flex';
    applyReturnVisibility();
    markSize();
    saveState();
  }
  ...
  /* ===== тихий маркер-невидимка: единственный возврат при скрытом чате ===== */
  ...
  function buildPanel() {
    const css = `
      ...
      #rggchat.rgg-wall .rgg-sz,#rggchat.rgg-wall .rgg-site-toggle,#rggchat.rgg-wall .rgg-wall-open{display:none}
      ...
      .rgg-site-toggle{position:relative;overflow:hidden}
      .rgg-site-toggle .site-ico{display:block;transition:.2s}
      ...
      .rgg-site-toggle.flash{animation:rggPinFlash .45s}
    ...
    if (MODE === 'dock') {
      if (st && st.hidden === true) setHidden(true);   // setHidden сам покажет тихий маркер
      else applyReturnVisibility();                    // чат виден — маркер скрыт
    }
    if (MODE === 'wall') {
  ...
  function buildDom() {
    try {
      ...
      const oldD = document.getElementById('rggdock'); if (oldD) oldD.remove();   // убираем якорь от v20 при апгрейде
